(function () {
                    ...
                    for (const dir of [-1, 1]) {
                        ...
                        if (maybeRook && maybeRook.type === "R" && maybeRook.color === color && !maybeRook.moved) {
                            ...
                            if (inBounds(r, kingDest) && inBounds(r, rookDest)) {
                                moves.push({r, f: kingDest, castle: true, rookFrom: { r, f: nf }, rookTo: { r, f: rookDest } });
                            }
                        }
                    }
                }
    ...
    toggleZenBtn.innerHTML = "🎬 Zen Mode";
    controlsRow.appendChild(toggleZenBtn);
    ...
    function onPointerDown(e) {
        ...
        if (piece && piece.color === turn) {
            ...
            dragCandidate = {r, f, pointerId: e.pointerId, startX: e.clientX, startY: e.clientY, moved: false, wasAlreadySelected, ghost: null, cellRect: cell.getBoundingClientRect(), pieceEl };
    ...
    function makeMove(fr, ff, move) {
        ...
        if (captured && captured.type === "K") {
            gameOver = true;
            gameOverText = (movingColor === "w" ? "White" : "Black") + " wins by capturing the King!";
        }
        ...
        if (movingType === "P") {
            const lastRank = movingColor === "w" ? SIZE - 1 : 0;
            if (tr === lastRank) {
                moving.type = "Q";
                wasPromotion = true;
            }
        ...
        history.push({
            ...
            turn: turn === "w" ? "b" : "w",
            lastMove: { from: { r: fr, f: ff }, to: { r: tr, f: tf } }
        });
    ...
    toggleZenBtn.addEventListener("click", () => {
        hideAllUi = !hideAllUi;
        toggleZenBtn.innerHTML = hideAllUi ? "📺 Show UI" : "🎬 Zen Mode";
        render();
        buildLabels();
    });
    ...
    zoomPresetBtn.addEventListener("click", () => {
        ...
        if (zoomPreset === 0) zoomPresetBtn.innerHTML = "🔍 Zoom: Standard (1)";
        else if (zoomPreset === 1) zoomPresetBtn.innerHTML = "🔍 Zoom: Clean Board (2)";
        else if (zoomPreset === 2) zoomPresetBtn.innerHTML = "🔍 Zoom: View Window (3)";
        buildLabels();
        render();
    });
    ...
    if (aiDifficulty) {
        aiDifficulty.addEventListener("change", (e) => {
            aiDepth = Number(e.target.value) || 2;
        });
        aiDepth = Number(aiDifficulty.value) || 2;
    }

    // ---- Initialize Game Engine ----
    board = freshBoard();
    history.push({ board: cloneBoard(board), turn: "w", lastMove: null });
    buildLabels();
    render();
})();
